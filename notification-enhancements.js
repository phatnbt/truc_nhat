(()=>{
  ...
  function newestRelevantSchedule(){
    ...
    return [...(state?.schedules||[])]
      .filter(schedule=>schedule?.weekStart&&parseDate(schedule.weekStart)>=monday)
  ...
  function actorKey(myId){
    return authSession?.user?.uid||myId||"member";
  }
  ...
  async function notifyNewSchedule(myId){
    ...
    await showNotification(
      "P708 · Có lịch trực tuần mới",
      `Tuần ${weekRange(schedule.weekStart)} · Việc của bạn: ${summary||"Mở ứng dụng để xem phân công"}.`
    );
  ...
  async function notifyWednesdayThursdayEvening(myId,now=new Date()){
    ...
    const schedule=scheduleForDate(now);
    if(!schedule)return;
    const pending=pendingAssignments(schedule,myId);
    if(!pending.length)return;
    const dayLabel=day===3?"Thứ 4":"Thứ 5";
    const tasks=pending.map(item=>item.task).filter(Boolean);
    const summary=tasks.length<=2?tasks.join(" · "):`${tasks.slice(0,2).join(" · ")} +${tasks.length-2} việc`;
    await showNotification(
      `P708 · Nhắc trực tối ${dayLabel}`,
      `Bạn còn ${pending.length} việc trực tuần này: ${summary}. Hãy hoàn thành và báo đã làm.`
    );
  }
  ...
  function nextEveningTarget(from=new Date()){
    for(let offset=0;offset<=7;offset++){
      ...
      candidate.setHours(EVENING_HOUR,0,0,0);
  ...
  function armEveningTimer(){
    ...
    const target=nextEveningTarget(new Date());
    if(!target)return;
    eveningTimer=setTimeout(async()=>{
      await runAutomaticReminders(false).catch(()=>{});
      armEveningTimer();
    },Math.max(1000,target.getTime()-Date.now()+1000));
  }
  ...
  async function enableAutomaticReminders(){
    ...
    if(permission==="granted"){
      ...
      updateNotificationButton();
      toast("Đã bật: lịch tuần mới · T4/T5 19:00 · kết toán");
      armEveningTimer();
  ...
  document.addEventListener("visibilitychange",()=>{
    if(!document.hidden){runAutomaticReminders(false).catch(()=>{});armEveningTimer();}
  });
